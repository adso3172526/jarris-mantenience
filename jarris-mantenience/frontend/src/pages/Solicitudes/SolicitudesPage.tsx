...
import ChangeAssetModal from '../WorkOrders/ChangeAssetModal';
...
const ACTIVE_STATUSES = ['NUEVA', 'ASIGNADA', 'EN_PROCESO', 'TERMINADA', 'RECHAZADA'];
...
const SolicitudesPage: React.FC = () => {
  ...
  const canCloseSolicitud = hasPermission('CERRAR_SOLICITUD');
  const canRejectSolicitud = hasPermission('RECHAZAR_SOLICITUD');
  ...
  useEffect(() => {
    applyFilters();
  }, [workOrders, searchText, filterStatus, filterLocation, filterAssignee, filterType, dateRange]);

  useEffect(() => {
    setMobilePage(1);
  }, [searchText, filterStatus, filterLocation, filterAssignee, filterType, dateRange]);
  ...
  const loadWorkOrders = async () => {
    try {
      ...
      // Filter only active statuses (not CERRADA)
      let activeOrders = response.data.filter(
        (wo: any) => ACTIVE_STATUSES.includes(wo.status)
      );
      ...
      if (!isJefe) {
        if (canSeeAllOT && !canSeeAllLocativo) {
          activeOrders = activeOrders.filter((wo: any) => wo.maintenanceType === 'EQUIPO');
        } else if (!canSeeAllOT && canSeeAllLocativo) {
          activeOrders = activeOrders.filter((wo: any) => wo.maintenanceType === 'LOCATIVO');
        }
      }
  ...
  const hasActiveFilters = !!(searchText || filterStatus || filterLocation || filterAssignee || filterType || dateRange);
  ...
  const iconBtnStyle = { padding: '0 4px', minWidth: 24, height: 24 };
  ...
  const getActionButtons = (record: WorkOrder, isMobileView = false) => {
    ...
    buttons.push(
      wrapTooltip("view", "Ver detalles",
        <Button
          ...
          block={isMobileView}
          style={!isMobileView ? iconBtnStyle : {}}
        >
          {isMobileView && "Ver"}
        </Button>
      )
    ...
    if (record.status !== 'RECHAZADA' && (isCreator || isJefe)) {
      buttons.push(
        wrapTooltip("photos", "Subir fotos",
          <Button
            ...
            style={!isMobileView ? { ...iconBtnStyle, color: '#722ed1' } : { color: '#722ed1' }}
            block={isMobileView}
          >
            {isMobileView && "Fotos"}
          </Button>
      ...
      buttons.push(
        wrapTooltip("assign", "Asignar",
          <Button
            ...
            block={isMobileView}
            style={isMobileView ? { color: '#faad14', background: '#fff' } : iconBtnStyle}
          >
    ...
    // Cambiar activo (solo JEFE, solo OT de EQUIPO)
    if (isJefe && record.maintenanceType === 'EQUIPO' && record.status !== 'RECHAZADA') {
      buttons.push(
        wrapTooltip("change-asset", "Activo",
          <Button
            ...
            style={!isMobileView ? { ...iconBtnStyle, color: '#1890ff' } : { color: '#1890ff' }}
            block={isMobileView}
          >
            {isMobileView && 'Activo'}
          </Button>
    ...
    // Cambiar ubicación
    if ((isJefe || hasPermission('CAMBIAR_UBICACION_OT')) && record.status !== 'RECHAZADA') {
      buttons.push(
      ...
      buttons.push(
        wrapTooltip("reassign", "Reasignar",
          <Button
            ...
            icon={<UserAddOutlined />}
            onClick={() => handleAssign(record)}
            style={!isMobileView ? { ...iconBtnStyle, color: '#faad14' } : { color: '#faad14' }}
            block={isMobileView}
          >
      ...
      buttons.push(
        wrapTooltip("start", "Iniciar",
          <Button
            ...
            style={!isMobileView ? { ...iconBtnStyle, color: '#52c41a' } : { backgroundColor: '#52c41a', borderColor: '#52c41a' }}
            block={isMobileView}
          >
      ...
      buttons.push(
        wrapTooltip("finish", "Finalizar",
          <Button
            ...
            style={!isMobileView ? { ...iconBtnStyle, color: '#faad14' } : { backgroundColor: '#faad14', borderColor: '#faad14' }}
            block={isMobileView}
          >
    ...
    // Cerrar - controlled by CERRAR_SOLICITUD
    if ((isJefe || canCloseSolicitud) && record.status === 'TERMINADA') {
      buttons.push(
        wrapTooltip("close", "Cerrar OT",
          <Button
            ...
            style={!isMobileView ? { ...iconBtnStyle, color: '#E60012' } : { color: '#E60012', background: '#fff' }}
            block={isMobileView}
          >
            {isMobileView && "Cerrar"}
          </Button>
    ...
    // Rechazar - controlled by RECHAZAR_SOLICITUD
    if ((isJefe || canRejectSolicitud) && record.status === 'NUEVA') {
      buttons.push(
        wrapTooltip("reject", "Rechazar",
          <Button
            ...
            block={isMobileView}
            style={isMobileView ? { color: '#ff4d4f', background: '#fff' } : iconBtnStyle}
          >
  ...
  const allColumns: ColumnsType<WorkOrder> = [
    ...
    },
    {
      ...
      render: (type) => (
        <Tag color={type === 'EQUIPO' ? 'blue-inverse' : 'purple-inverse'} style={{ fontSize: 11, margin: 0, width: 82, textAlign: 'center', display: 'inline-block' }}>
          {type === 'EQUIPO' ? (
            <><ToolOutlined /> EQUIPO</>
          ) : (
            <><HomeOutlined /> LOCATIVO</>
    ...
    },
    {
      ...
      render: (priority: string) => priority ? (
        <Tag style={{ backgroundColor: workOrderPriorityStyles[priority]?.bg, color: workOrderPriorityStyles[priority]?.color, border: 'none', fontSize: 11, margin: 0 }}>
    ...
    },
    {
      ...
      render: (status) => (
        <Tag style={{ backgroundColor: workOrderStatusStyles[status]?.bg, color: workOrderStatusStyles[status]?.color, border: 'none', fontSize: 11, margin: 0 }}>
          {status}
        </Tag>
      ),
    },
  ...
  const hiddenForPDV = ['assignee', 'cost', 'materialsCost', 'total'];
  const columns = isPDV
        ...
        title={
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            <Space>
              ...
              <span style={{ fontSize: isMobile ? 14 : 16, fontWeight: 600 }}>Solicitudes</span>
            </Space>
      ...
      >
        ...
        {isMobile ? (
          <div style={{ marginBottom: 12 }}>
            ...
            >
              Filtros{hasActiveFilters ? ` (${[searchText, filterStatus, filterAssignee, filterLocation, filterType, dateRange].filter(Boolean).length})` : ''}
            </Button>
            {filtersOpen && (
              <Card size="small" style={{ marginTop: 8, background: '#fafafa' }}>
                ...
                {hasActiveFilters && (
                  <div style={{ marginTop: 8, color: '#1890ff', fontSize: 11 }}>
                    Mostrando {filteredOrders.length} de {workOrders.length} solicitudes
                  </div>
                )}
              </Card>
            )}
